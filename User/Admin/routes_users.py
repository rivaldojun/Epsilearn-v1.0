...
from User.Admin.route_studenthub_admin import AdminBp
import random

#GESTION USERS
@AdminBp.route('/validerprof/<int:id>')
def validerprof(id):
...
@AdminBp.route('/get_etudiants', methods=['GET'])
def get_etudiants():
    if session.get('admin')=='connect':
        try:
...
@AdminBp.route('/get_user', methods=['GET'])
def get_user():
    if session.get('admin')=='connect':
        try:
