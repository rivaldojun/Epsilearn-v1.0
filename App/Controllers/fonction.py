...
def save_modifier_profil_picture(file,ob,us):
    current_date = datetime.now()
    date_string = current_date.strftime("%Y%m%d%H%M%S")
    file_extension = file.filename.split('.')[-1]
    filename = f"{secure_filename(file.filename.replace('.', '_'))}_{date_string}.{file_extension}"
    path1="App/static"
    path2="Profil"
    file.save(os.path.join(path1,path2, filename))
    ob.photo =os.path.join("static",path2, filename)
    us.photo =os.path.join("static",path2, filename)
